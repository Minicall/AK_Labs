...
MODULE_DESCRIPTION("Laboratory3");
MODULE_LICENSE("GPL");

static uint sprob = 1;

module_param(sprob, uint, 0444);
MODULE_PARM_DESC(sprob, "Number of Hello");
...
static int __init start_module(void)
{
    ...
    if (sprob == 0) {
        printk(KERN_WARNING "The sprob parameter is 0\n");
       
    } else if (sprob > 10) {
        printk(KERN_WARNING "The sprob parameter exceeds 10\n");
        return -EINVAL;
    } else if (sprob > 5) {
        printk(KERN_WARNING "The sprob parameter between 5 & 10\n");
        return -EINVAL;
    }
    
    for (idx = 0; idx < sprob; idx++) {
